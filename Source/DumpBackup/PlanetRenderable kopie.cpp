 ...
 *
 *  Based on TerrainRenderable.cpp (LGPL)
 *
 */
#halt
...
#define DELTA_BINDING 1
    //-----------------------------------------------------------------------
    //-----------------------------------------------------------------------
    String PlanetRenderable::mType = "PlanetTile";
    //-----------------------------------------------------------------------
    //-----------------------------------------------------------------------
    
    //-----------------------------------------------------------------------
    PlanetRenderable::PlanetRenderable(const String& name, OctreeSceneManager* sceneManager)
    : Renderable(), MovableObject(name), mSceneManager(sceneManager), mPositionBuffer(0)
    {
	...
	//-----------------------------------------------------------------------
	uint32 PlanetRenderable::getTypeFlags(void) const
	{
		// return world flag
		return SceneManager::WORLD_GEOMETRY_TYPE_MASK;
	}
    //-----------------------------------------------------------------------
    void PlanetRenderable::deleteGeometry()
    {
        if(mPlanet)
            OGRE_DELETE mPlanet;
    ...
    //-----------------------------------------------------------------------
    void PlanetRenderable::initialise(int startx, int startz,  
                                       Real* pageHeightData) {
        ...
        mPlanet = OGRE_NEW VertexData;
        mPlanet->vertexStart = 0;
        mPlanet->vertexCount = mOptions->tileSize * mOptions->tileSize;
        
        VertexDeclaration* decl = mPlanet->vertexDeclaration;
        VertexBufferBinding* bind = mPlanet->vertexBufferBinding;
        ...
        mPositionBuffer = OGRE_ALLOC_T(float, mPlanet->vertexCount * 3, MEMCATEGORY_GEOMETRY);
